...
class AttentionMechanism(nn.Module):
    def __init__(self, input_shape, message_shape, args, n_agents, trajectory_length=10):
        ...
        self.input_shape = input_shape
        self.tau_shape = input_shape + args.n_actions
        self.message_shape = message_shape
        self.args = args
        self.n_agents = n_agents
        self.trajectory_length = trajectory_length
        ...
        self.qs = nn.Linear(message_shape*self.n_agents, self.args.hidden_dim)
        self.ks = nn.Linear(self.tau_shape*trajectory_length, self.args.hidden_dim)
        self.vs = nn.Linear(self.tau_shape*trajectory_length, self.args.hidden_dim)


    def forward(self, imagined_trajectory, received_messages):
        # need to duplicate messages for every agent to see
        received_messages = torch.stack([received_messages.flatten()], 0)

        qs = self.qs(received_messages)
        ks = self.ks(imagined_trajectory)
        vs = self.vs(imagined_trajectory)
...
class RNNNSCommunicatingAgent(nn.Module):
    ...
    def forward(self, inputs, hidden_state):
        ...
        # q here should contain the messages too, so it should have shape (n_agents, n_actions + message_shape)
        return q, h
